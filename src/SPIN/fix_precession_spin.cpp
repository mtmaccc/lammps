  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (mask[i] & groupbit) {
      ...
      if (elastic_flag) {        // compute MagnetoElastic anisotropy
	      int j,jj,jnum,itype,jtype,icomp;
	      double xi[3], sio[3], sjo[3], rij[3], rijo[3];	
	      double a[3][3], c[3][3], l[3][3], eij[3][3];
	      double delx, dely, delz,nearest;
	      double local_cut2, next_cut2, rsq;
	      int *jlist;
	      double **x = atom->x;
	      double **f = atom->f;
       ...
       solve3x3exactly(a,c,l);
        ...
        //printf("Current strain on atom i = %d e1 =%.16f e2 =%.16f e3 =%.16f e4 =%.16f e5 =%.16f e6 =%.16f   \n ",icomp ,eij[0][0] ,eij[1][1],eij[2][2],eij[2][1], eij[2][0], eij[1][0]);
	      ...
	      compute_elastic(eij,spi,fmi,nearest);
        double magEnergy = -compute_elastic_energy(eij,spi,nearest);
        epreci -= magEnergy;
	      //epreci -= compute_elastic_energy(eij,spi,nearest);
	      
	      compute_elastic_mech(spi,fi,nearest); // fix eventually 
        ...
        if (evflag) {
          double unwrap[3];
          domain->unmap(x[i], image[i], unwrap);  // Account for PBCs
        
          // Virial tensor components (negative sign for internal forces)
          double v[6] = {
            fi[0] * (unwrap[0]),
            fi[1] * (unwrap[1]),
	          fi[2] * (unwrap[2]),
            ...
            /*-fi[0] * unwrap[0],  // xx: -F_x * r_x
            -fi[1] * unwrap[1],  // yy: -F_y * r_y
            -fi[2] * unwrap[2],  // zz: -F_z * r_z
            -fi[0] * unwrap[1],  // xy: -F_x * r_y (shear yx)
            -fi[0] * unwrap[2],  // xz: -F_x * r_z (shear zx)
            -fi[1] * unwrap[2]   // yz: -F_y * r_z (shear zy)*/
          };
        
         //v_tally(i, v);  // Thread-safe virial accumulation
         ev_tally(1, jlist, 1, magEnergy, v);
         //v_tally(1, jlist,1, v);
        }
         //printf("Spin direction on atom i = %d ,sx=%.16f, sy =%.16f, sz =%.16f  \n ",icomp,spi[0],spi[1],spi[2]);
         //printf("Current strain on atom i = %d e1 =%.16f e2 =%.16f e3 =%.16f e4 =%.16f e5 =%.16f e6 =%.16f   \n ",icomp ,eij[0][0] ,eij[1][1],eij[2][2],eij[2][1], eij[2][0], eij[1][0]);
         //printf("Magnetoelastic energy on atom i = %d is %.16f w/ %f nearest atoms \n",i,epreci,nearest);
...
void FixPrecessionSpin::compute_single_precession(int i, double spi[3], double fmi[3])
{
  ...
  if (mask[i] & groupbit) {
    ...
    if (elastic_flag){
      ...
      double **f = atom->f;
      double **sp = atom->sp;
      double local_cut2, next_cut2, nearest;
      ...
      compute_elastic(eij,spi,fmi,nearest);
         //printf("Spin direction on atom (SINGLE PRECESSION) i = %d ,sx=%.16f, sy =%.16f, sz =%.16f  \n ",icomp,spi[0],spi[1],spi[2]);
         //printf("Current strain on atom (SINGLE PRECESSION)i = %d e1 =%.16f e2 =%.16f e3 =%.16f e4 =%.16f e5 =%.16f e6 =%.16f   \n ",icomp ,eij[0][0] ,eij[1][1],eij[2][2],eij[2][1], eij[2][0], eij[1][0]);
...
void FixPrecessionSpin::compute_elastic(double eij[3][3], double spi[3], double fmi[3], double nearest)
{
  double skx,sky,skz;
  ...
  //b1bar =(b1eh)/(nearest/8);
  //b2bar =(b2eh)/(nearest/8);
  b1bar =(b1eh);
  b2bar =(b2eh);
  //printf("B1/hbar =%.16f,B2/hbar=%.16f,  \n ",b1bar, b2bar);
  ...
  firstx = b1bar * (nc1x * first1 + nc2x * first2 + nc3x * first3);
  firsty = b1bar * (nc1y * first1 + nc2y * first2 + nc3y * first3);
  firstz = b1bar * (nc1z * first1 + nc2z * first2 + nc3z * first3);
  ...
  secondx = b2bar * (nc1x * second1 + nc2x * second2 + nc3x * second3);
  secondy = b2bar * (nc1y * second1 + nc2y * second2 + nc3y * second3);
  secondz = b2bar * (nc1z * second1 + nc2z * second2 + nc3z * second3);
  //Sum two parts together and add into forces
  //Note** assumes n1,n2, and n3 are cardinal directions. Fix later?
  //printf("effective field on atom ,xfirst=%.16f,xsecond=%.16f, yfirst =%.16f, ysecond = %.16f zfirst =%.16f zsecond = %.16f  \n ",firstx,secondx,firsty,secondy,firstz,secondz);
  //printf("effective field on atom , fmx=%.16f, fmy= %.16f, fmz= %.16f\n ",firstx+secondx,firsty+secondy,firstz+secondz);
  fmi[0] += (firstx + secondx);
  fmi[1] += (firsty + secondy);
  fmi[2] += (firstz + secondz);
}
/* ---------------------------------------------------------------------- */
void FixPrecessionSpin::compute_elastic_mech(double spi[3], double fi[3], double nearest)
{
  double skx,sky,skz,skx2,sky2,skz2;
...
/* ----------------------------------------------------------------------*/ 
double FixPrecessionSpin::compute_elastic_energy(double eij[3][3], double spi[3], double nearest)
{
  double skx,sky,skz;
...
double FixPrecessionSpin::compute_vector(int n)
{
  // only sum across procs one time

  if (eflag == 0) {
    MPI_Allreduce(fi, fi_all, 3, MPI_DOUBLE, MPI_SUM, world);
    eflag = 1;
  }
  return fi_all[n + 1];
}
